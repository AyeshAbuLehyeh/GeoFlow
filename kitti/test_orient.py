...
def evaluate_with_orientation(model, test_loader, device, rotation_range=10.0, shift_range_lat=20.0, shift_range_lon=20.0):
    ...
    with torch.inference_mode():
        for sat_map, camera_k, grd_img, x_shift, y_shift, theta_normalized, file_name in tqdm(
            test_loader, total=len(test_loader), desc="Evaluating (orient)", unit="batch"
        ):
