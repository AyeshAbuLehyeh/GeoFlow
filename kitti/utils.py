...
CameraGPS_shift_right = [1.08, 0.8]
...
def gps2shiftmeters(latlon):
    ...
    shift_x = (latlon[:, :1, 1] - latlon[:, :, 1]) * kx
    shift_y = (latlon[:, :, 0] - latlon[:, :1, 0]) * ky
    shift = torch.cat([shift_x.unsqueeze(-1), shift_y.unsqueeze(-1)], dim=-1)
