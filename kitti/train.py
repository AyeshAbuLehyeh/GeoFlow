...
try:
    ...
    from .losses import AngularDirectionLoss
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train GeoFlow on KITTI (no orientation head)")
...
def main():
    ...
    run_name = f"kitti_{args.backbone.replace('_', '')}_d{args.d_model}_{timestamp}"
    run_dir = os.path.join(args.checkpoint_dir, run_name)
    os.makedirs(run_dir, exist_ok=True)
    os.makedirs(args.log_dir, exist_ok=True)

    train_loader = load_train_data(file_path=args.train_list, batch_size=args.batch_size, use_augmentation=args.use_augmentation)
    ...
    direction_loss_fn = AngularDirectionLoss()
    ...
    with open(log_path, "w", encoding="utf-8") as log_file:
        log_file.write("GeoFlow KITTI training run\n")
        ...
        for epoch_idx in range(args.epochs):
            ...
            log_message = (
                ...
                f"r={metrics['r']:.6f} | lr_main={optimizer.param_groups[1]['lr']:.3e} | "
            ...
            state_path = os.path.join(run_dir, "last.pth")
            save_checkpoint(state_path, model, optimizer=optimizer, scheduler=scheduler, epoch=epoch_idx + 1, best_metric=best_train_loss, args=args)
